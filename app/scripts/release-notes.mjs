 ...
 * garantert innholdsløst: v0.6.0-beta.2, beta.3 og beta.4 bærer alle den samme «Alt innebygd … Se CHANGELOG.md for hva som er nytt» — en peker operatøren ikke kan klikke på.
 *
 ...
 * ## Bruk
 *
 *   node app/scripts/release-notes.mjs --check         # vakten, kjørbar for hånd
 *   node app/scripts/release-notes.mjs --emit v1.2.3   # skriver GITHUB_OUTPUT-blokk
 *
 * `app/scripts/release-notes.test.mjs` er den samme vakten som test, og
...
export function packageVersion() {
  return readJson("app/package.json").version;
}

/** Versjonen Tauri stempler inn i bundlen og i `latest.json`. */
export function tauriVersion() {
  return readJson("app/src-tauri/tauri.conf.json").version;
}
...
export function checkAll() {
  ...
  if (packageVersion() !== tauri) {
    problems.push(
      `app/package.json er ${packageVersion()} og app/src-tauri/tauri.conf.json er ${tauri} — taggen bygget lages fra må være den notatet er skrevet for`,
    );
  }
