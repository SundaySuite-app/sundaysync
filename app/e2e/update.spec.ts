...
test.describe("update check + download", () => {
  test("up to date: the check reports it and offers no download button", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Download and install/ }),
    ).toBeHidden();
  });

  test("an available update offers Download; live progress then Restart", async ({
    page,
  }) => {
    await openSettings(page, {
      ...BOOT_FIXTURES,
    ...
    await page.evaluate(() =>
      (window as any).__SUNDAYSYNC_RESOLVE_DOWNLOAD__(),
    );
    await expect(page.getByText(en.updateReady("0.3.0"))).toBeVisible();
    await expect(
      page.getByRole("button", { name: en.updateRestart }),
    ).toBeVisible();
  });
  ...
  test("a failed download surfaces the error, and re-checking lets the operator retry", async ({
    ...
    await openSettings(page, {
      ...BOOT_FIXTURES,
      update_check: { phase: "available", version: "0.3.0" },
      update_download_install: fn(
        `() => Promise.reject("network unreachable")`,
      ),
    });

    await page.getByRole("button", { name: en.updateCheck }).click();
    await page
      .getByRole("button", { name: en.updateDownload("0.3.0") })
      .click();

    await expect(
      page.getByText(en.updateError("network unreachable")),
    ).toBeVisible();
    ...
    await expect(page.getByText(en.updateAvailable("0.3.0"))).toBeVisible();
    await expect(
      page.getByRole("button", { name: en.updateDownload("0.3.0") }),
    ).toBeVisible();
  });
});
...
test.describe("release notes in the update dialog (D-095)", () => {
  ...
  test("an offer without a note shows nothing — no heading, no empty box", async ({
    page,
  }) => {
...
test.describe("beta ring ↔ consent re-surface", () => {
  test("turning on beta re-opens consent for an undecided install", async ({
    page,
  }) => {
  ...
  test("turning on beta does not re-nag an install that already decided", async ({
    page,
  }) => {
    await openSettings(page, {
      ...BOOT_FIXTURES,
