...
test.describe("consent card", () => {
  test("appears when this install has not answered yet", async ({ page }) => {
    await boot(page, {
      fixtures: { ...UNDECIDED, ...consentSetSpy() },
  ...
  test("does not appear once this install has already decided", async ({
    page,
  }) => {
  ...
  test("«Yes, happy to help» grants consent and dismisses the card", async ({
    page,
  }) => {
    await boot(page, {
      fixtures: { ...UNDECIDED, ...consentSetSpy() },
  ...
  test("«No thanks» declines and dismisses the card just the same", async ({
    page,
  }) => {
    await boot(page, {
      fixtures: { ...UNDECIDED, ...consentSetSpy() },
...
test.describe("telemetry settings section", () => {
  ...
  test("the toggle reflects the backend and writes back to it", async ({
    page,
  }) => {
    ...
    const telemetrySection = page.locator(".field", {
      hasText: en.telemetryTitle,
    });
  ...
  test("«Show what we send» renders the preview payload", async ({ page }) => {
    const payload = JSON.stringify(
      { installId: "e2e-install", counters: [] },
      null,
      2,
    );
    await openSettings(page, {
  ...
  test("a failing preview command fails soft to the empty-state copy", async ({
    page,
  }) => {
    await openSettings(page, {
      ...BOOT_FIXTURES,
  ...
  test("«Delete my data» asks for confirmation, then calls the deletion command", async ({
    ...
    await expect
      .poll(() => page.evaluate(() => (window as any).__E2E_DELETE_CALLED__))
      .toBe(true);
