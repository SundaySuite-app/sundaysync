...
describe("playback memory: long-session simulation (V03-S7)", () => {
  it("keeps resident bytes within the 256 MB budget across a 3-hour, 10-device sweep, and never evicts a chunk that is still needed", async () => {
    ...
    for (let tSec = 0; tSec <= CLIP_DURATION_SEC; tSec += STEP_SEC) {
      ...
      for (const clip of clips) {
        ...
        const nowChunkIndex = Math.floor(
          (tSec * ANALYSIS_RATE) / CHUNK_SAMPLES,
        );
    ...
    const totalChunksTouched =
      CLIP_COUNT *
      Math.ceil((CLIP_DURATION_SEC * ANALYSIS_RATE) / CHUNK_SAMPLES);
  ...
  it("a single ~90-minute clip fits the whole way through without ever needing an eviction", async () => {
    ...
    const totalChunks = Math.ceil(
      (clip.durationSec * ANALYSIS_RATE) / CHUNK_SAMPLES,
    );
